...
y2=`gawk -v w=$w -v h=$h -v nw=$nw -v nh=$nh  'BEGIN{print -0.75*w}'`
...
gawk 'BEGIN { for(ix=1.1; ix<=4.9; ix=ix+0.1) { print ix,1 ; print ix,5 ; print ">"} ; 
              for(iy=1.1; iy<=4.9; iy=iy+0.1) { print 1,iy ; print 5,iy ; print ">"} }' | psxy -R -JX -W3px/255/240/240 -K -P -O -m -Bg1 >> $psfile
