...
---
**if apply $\delta G $ on initial G instead of $g^0_{qkj} $ then:**(reverse derive)
<script type="math/tex; mode=display">
\begin{aligned}
...
\Delta u(\xi_1,\xi_2) &=\sum_{k=1}^k\sum_{l=1}^La_{jkl}\Phi_{kl}(\xi_1,\xi_2)       \color{rgb}  \impliedby \Phi_{kl}(\xi_1,\xi_2)=N_k(\xi_1)N_l(\xi_2) \\
...
if considering uncertain of Green Function( $\delta G $ ), then follows the same formulation derivation above with seismic data
