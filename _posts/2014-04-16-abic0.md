...
\begin{matrix}
 ...
 \bbox[margin:0px 0px 0px 15px;  ] {\frac{1}{c^2}}  & \mathbf{ \bbox[padding:23px;  border:2px solid black] { S_\tau} }
\end{matrix}
}
\mathbf{\bbox[padding:25px 2px; border:2px solid black] {a}}  =
\begin{matrix}
\mathbf{\bbox[padding:25px 2px; border:2px solid black] {0}} &,  \rho_\xi^2\\
\mathbf{\bbox[padding:25px 2px; border:2px solid black] {0}} &,  \frac{1}{c^4}\rho_\tau^2 
\end{matrix} 
$$</div>
...
f(\mathbf{Sa})&= \cdots exp \left[ -\frac{1}{2} \mathbf{a}^T \begin{pmatrix}  S_\xi^T & S_\tau^T \end{pmatrix} \begin{pmatrix} \rho_\xi^2 \mathbf{I_M} & 0 \\ 0 & \rho_\tau^2 \mathbf{I_M} \end{pmatrix}^{-1}  \begin{pmatrix} S_\xi \\ S_\tau \end{pmatrix} \mathbf{a} \right] \\
              &= \cdots exp \left[-\frac{1}{2} \mathbf{a}^T \left( \frac{S_\xi^TS_\xi+\chi^2S_\tau^TS_\tau}{\rho^2} \right) \mathbf{a}\right], \quad \chi^2=\frac{\rho_\xi^2}{\rho_\tau^2}= \frac{1}{c^4} \\

f(\mathbf{Sa})&= \cdots exp \left[ -\frac{1}{2} \mathbf{a}^T \begin{pmatrix}  S_\xi^T & \frac{1}{c^2}S_\tau^T \end{pmatrix} \begin{pmatrix} \rho_\xi^2 \mathbf{I_M} & 0 \\ 0 & \frac{1}{c^4}\rho_\tau^2 \mathbf{I_M} \end{pmatrix}^{-1}  \begin{pmatrix} S_\xi \\ \frac{1}{c^2}S_\tau \end{pmatrix} \mathbf{a} \right] \\
              &= \cdots exp \left[-\frac{1}{2} \mathbf{a}^T \left( \frac{S_\xi^TS_\xi+\chi^2S_\tau^TS_\tau}{\rho^2} \right) \mathbf{a}\right], \quad \chi^2=\frac{\rho_\xi^2}{\rho_\tau^2}= \frac{1}{c^4} \\
...
\color{red}P&{\color{red}(\mathbf{a};\sigma_g^2,\sigma_b^2,\rho^2|\mathbf{d})}=cP(\mathbf{d}|\mathbf{a};\sigma_g^2,\sigma_b^2)P(\mathbf{a};\rho^2)\\
 ...
 &=\bbox[#A9F5A9]{c(2\pi\sigma_g^2)^{-(N+M')/2}\|\Sigma_d(\gamma^2,{\color{red}\mathbf{a}^i})\|^{-1/2} \|\beta^2(G_\xi+\chi^2G_\tau)\|^{1/2} } \\
 ...
 &=\bbox[#A9F5A9]{[\cdots]}\times exp \left[-\frac{1}{2\sigma_g^2} \bbox[#F5A9BC]{\mathbf{S(a)}} \right] ;\quad \gamma^2=\sigma_b^2/\sigma_g^2;\quad \beta^2=\sigma_g^2/\rho^2;\quad \sigma_g^2\Sigma_d=C_d 
\end{aligned}
$$</div>
...
<div>$$
\begin{aligned}
L\left(\begin{matrix}\sigma_g^2,&\sigma_b^2,&\rho^2 \\ \sigma_g^2,& \gamma^2,&\beta^2\end{matrix};{\color{red}\mathbf{a}^i}\right)
=\int P(\mathbf{a};\sigma_g^2,\sigma_b^2,\rho^2|\mathbf{d})~d\mathbf{a}
=\int \bbox[#A9F5A9]{[\cdots]}\times exp \left[-\frac{1}{2\sigma_g^2} \bbox[#F5A9BC]{\mathbf{S(a)}} \right]  ~d\mathbf{a}
\end{aligned}
$$</div>
...
S(\mathbf{a}) & \textit{taylor expansion at :}\; a^\star  \\
S(\mathbf{a})&\approx S(\mathbf{a}^\star) + J(\mathbf{a^\star}) (\mathbf{a-a^\star}) +\frac{1}{2} (\mathbf{a-a^\star})^\mathrm{T} H(\mathbf{a^\star}) (\mathbf{a-a^\star}); \quad J(a)|_{a=a^\star}=0 \\
...
<div>$$
\begin{aligned}
L&(\sigma_g^2,\gamma^2,\beta^2;{\color{red}\mathbf{a}^i})= \bbox[#A9F5A9]{[\cdots]}\times exp \left[-\frac{1}{2\sigma_g^2} \mathbf{S(a^\star)} \right] \\
 ...
 &=c(2\pi {\color{#0000FF}\sigma_g^2})^{-(N+M'-M'')/2} \times exp \left[-\frac{1}{2{\color{#0000FF}\sigma_g^2}} \mathbf{S(a^\star)} \right] \times \|\sim\|^{-1/2}~\|\sim\|^{1/2}~\|\sim\|^{-1/2}
\end{aligned}
$$</div>
...
with&:\\
~\\
S(\mathbf{a}^\star)&=(\mathbf{d-Ha^\star})^T\mathbf{\Sigma}_d^{-1}({\color{red}\gamma^2},{\mathbf{a}^i})(\mathbf{d-Ha^\star}) + {\color{red}\beta^2}\mathbf{a^\star}^T ( G_\xi+\chi^2G_\tau ) \mathbf{a^\star}\\
...
C_a &=\sigma_g^2\left[\mathbf{H}^T\Sigma_d^{-1}({\color{red}\gamma^2},\mathbf{a}^i)\mathbf{H}+{\color{red}\beta^2}(\mathbf{G_\xi+\chi^2G_\tau})\right]^{-1} \\
\textit{iteratively }&\textit{update initial model:} \\
a_n^i&=a_{n-1}^i+ w(a_{n-1}^\star - a_{n-1}^i) (0 \lt w \lt 1) \\
&~\\
&~\\
\mathbf{a}^\star({\color{red}\sigma_b^2},{\color{red}\rho^2};a^i)&=\left[\mathbf{H}^T C_d^{-1}({\color{red}\sigma_b^2},\mathbf{a}^i) \mathbf{H}+{(\mathbf{G_\xi+\chi^2G_\tau})}/{\color{red}\rho^2}\right]^{-1}\mathbf{H}^T C_d^{-1}({\color{red}\sigma_b^2},\mathbf{a}^i) \mathbf{d} \\
C_a &=\left[\mathbf{H}^T~C_d^{-1}({\color{red}\sigma_b^2},\mathbf{a}^i)\mathbf{H}+{(\mathbf{G_\xi+\chi^2G_\tau})}/{\color{red}\rho^2}\right]^{-1} \\
\end{aligned}
$$</div>
...
<div>$$
\begin{aligned}
         ~ & \left(\mathbf{H}^T\Sigma_d^{-1}\mathbf{H}+\mathbf{S}^T\Sigma_s^{-1}\mathbf{S}\right)\mathbf{a}=\mathbf{H}^T\Sigma_d^{-1}\mathbf{d}\\
\mathbf{a}=&\left(\mathbf{H}^T\Sigma_d^{-1}\mathbf{H}+\mathbf{S}^T\Sigma_s^{-1}\mathbf{S}\right)^{-1}\mathbf{H}^T\Sigma_d^{-1}\mathbf{d}\\
\end{aligned}
$$</div>

<div>$$
\begin{aligned}
\Sigma_a& = \left[\left(\mathbf{H}^T\Sigma_d^{-1}\mathbf{H}+\mathbf{S}^T\Sigma_s^{-1}\mathbf{S}\right)^{-1}\mathbf{H}^T{\Sigma_d^{-1}} \right]  \cdot {\Sigma_d} \cdot  
            \left[\left(\mathbf{H}^T\Sigma_d^{-1}\mathbf{H}+\mathbf{S}^T\Sigma_s^{-1}\mathbf{S}\right)^{-1} \mathbf{H}^T\Sigma_d^{-1} \right]^T \\
        & = \left(\mathbf{H}^T\Sigma_d^{-1}\mathbf{H}+\mathbf{S}^T\Sigma_s^{-1}\mathbf{S}\right)^{-1}  \cdot \left[\left(\mathbf{H}^T\Sigma_d^{-1}\mathbf{H}+\mathbf{S}^T\Sigma_s^{-1}\mathbf{S}\right)^{-1} \mathbf{H}^T\Sigma_d^{-1} \mathbf{H}\right]^T \\
        & = \left(\mathbf{H}^T\Sigma_d^{-1}\mathbf{H}+\mathbf{S}^T\Sigma_s^{-1}\mathbf{S}\right)^{-1} \quad; \mathbf{a}=(\cdots)\mathbf{d}=(\cdots)\mathbf{Ha}\quad  \Rightarrow \quad (\cdots)H \overset{def}{\iff} \mathbf{I} \\
\end{aligned}
$$</div>
</div>
...
<div>$$
\begin{aligned}
\dot{D}_q^0(t,\xi)  \thickapprox & \sum_{k=1}^K \sum_{l=1}^L a_{qkl}X_k(\xi)T_l(t-t_k) \quad;\quad \int_\Sigma \int_t ~ \dot{D}_q^0(t,\xi) ~dt~d\xi \thickapprox \sum_{k=1}^K \sum_{l=1}^L a_{qkl}  \Delta \xi_1 \Delta \xi_2   \\
